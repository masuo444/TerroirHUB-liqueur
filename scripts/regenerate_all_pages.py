...
try:
    from liqueur_filter import is_liqueur_item as _genre_ok
except Exception:
    def _genre_ok(_n):
        return True
try:
    RAKUTEN_DB = json.load(open(os.path.join(BASE, 'liqueur', 'rakuten_items.json'), encoding='utf-8'))
except Exception:
    RAKUTEN_DB = {}
try:
    FURUSATO_LINKS = json.load(open(os.path.join(BASE, 'liqueur', 'furusato_links.json'), encoding='utf-8'))
except Exception:
    FURUSATO_LINKS = {}
...
<section class="section" style="background:#fff;" id="furusato">
  <div class="sec-inner">
    ...
    <p class="fz-lead">このメーカーの商品は、<b>{muni}</b>のふるさと納税返礼品として提供されています（{fl['count']}件）。
  ...
  <div class="sec-inner">
    ...
    if _lat and _lng:
        try:
            ...
            visit_items += (f'<div style="border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-top:18px;">'
