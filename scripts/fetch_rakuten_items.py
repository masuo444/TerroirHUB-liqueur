...
「蔵の商品グリッド」を取得し liqueur/rakuten_items.json に保存する。
...
from liqueur_filter import is_liqueur_item   # リキュール・梅酒以外を弾く共通フィルタ

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
CFG = json.load(open(os.path.join(BASE, 'scripts', 'rakuten_config.json'), encoding='utf-8'))
ENDPOINT = 'https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260701'
OUT_PATH = os.path.join(BASE, 'liqueur', 'rakuten_items.json')
...
def items_from(data):
    ...
    for w in data.get('Items', []):
        ...
        if not is_liqueur_item(name):   # リキュール・梅酒以外は除外
            continue
        rec = {
            'name': name,
            'image': img,
