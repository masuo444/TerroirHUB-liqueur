#!/usr/bin/env python3
"""県一覧ページ（liqueur/{pref}/index.html）にポータルの県別テロワールページへの導線を追加。

/terroir/ 48ページがジャンルサイトから被リンクゼロで孤立していたための内部リンク施策（2026-07-17、sakeに続く第2弾）。
...
def block(pref, name):
    ...
    return (f'<div style="max-width:1080px;margin:36px auto 0;padding:0 24px;text-align:center;">'
            f'<a href="{url}" style="font-size:13px;color:#C4547A;text-decoration:none;letter-spacing:0.03em;">'
...
def main():
    ...
    for pref, name in PREF_JA.items():
        path = os.path.join(BASE, 'liqueur', pref, 'index.html')
