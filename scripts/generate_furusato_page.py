...
ふるさと納税ページ生成（SEO重視の3層構造）。
  ...
  2) 県別: wine/furusato/{pref}.html （「山梨 リキュール ふるさと納税」等を県ごとに獲る）
...
from liqueur_filter import is_liqueur_item

TODAY = datetime.date.today().isoformat()

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DOMAIN = "liqueur.terroirhub.com"
OUTDIR = os.path.join(BASE, "liqueur", "furusato")
...
# 表示順: リキュールの主要産地を上位に。残りは返礼品数の多い順。
GENRE_PRIORITY = ['wakayama', 'nara', 'fukuoka', 'oita']
...
def load_wineries():
    m = {}
    for jf in glob.glob(os.path.join(BASE, "data/data_*_liqueurs.json")):
...
def collect():
    rk = json.load(open(os.path.join(BASE, "liqueur", "rakuten_items.json"), encoding="utf-8"))
    ...
    for wid, g in rk.items():
        ...
        for it in g.get("items", []):
            nm = it.get("name", "")
            if not it.get("image") or "ふるさと納税" not in nm or not is_liqueur_item(nm):
                continue
    ...
    return f'''
        <article class="fcard" data-kw="{kw}" data-price="{price or 0}" data-band="{band}">
          ...
          <div class="fcard-body">
            <a class="fcard-winery" href="/liqueur/{esc(it.get('pref',''))}/{esc(it.get('winery_id','').split(':')[-1])}.html">{esc(it['winery'])} ›</a>
            <h3 class="fcard-name" title="{esc(it['name'])}">{esc(nm)}</h3>
            {price_html}
            <a class="fcard-btn" href="{esc(it['url'])}" target="_blank" rel="nofollow sponsored noopener" onclick="if(window.gtag)gtag('event','furusato_click',{{genre:'liqueur'}});">楽天ふるさと納税で寄付 →</a>
          </div>
        </article>'''


FAQ_COMMON = [
    ("ふるさと納税でリキュールを選ぶメリットは？",
     "実質自己負担2,000円で、寄付額に応じた返礼品としてリキュールを受け取れます。さらに地域の小規模メーカーを直接応援できるのが魅力です。控除上限額は年収・家族構成で変わるため、各ふるさと納税サイトのシミュレーターで確認してから寄付しましょう。"),
    ("掲載のリキュールはどこで寄付できますか？",
...
<meta property="og:image" content="https://liqueur.terroirhub.com/ogp.png">
...
<nav class="nav">
  <div class="nav-top">
    <a href="/" class="nav-logo">Terroir HUB <span style="color:#4A8BAE">LIQUEUR</span></a>
    <a href="/liqueur/furusato/" class="nav-back">&#8592; ふるさと納税トップ</a>
  </div>
</nav>
"""
...
FOOTER = """
<footer class="footer">
  <div class="footer-logo">Terroir HUB <span class="accent">LIQUEUR</span></div>
  <nav class="footer-nav">
    <a href="/">ホーム</a>
    <a href="/liqueur/search/">メーカー検索</a>
    <a href="/liqueur/furusato/">ふるさと納税</a>
    <a href="/liqueur/guide/">リキュールガイド</a>
    <a href="/privacy.html">プライバシー</a>
    <a href="/terms.html">利用規約</a>
  </nav>
  <p class="footer-copy">&copy; 2026 合同会社FOMUS — Terroir HUB LIQUEUR</p>
</footer>
</body>
</html>
"""
...
NOTE = ('<p class="note">※ 返礼品・寄付額・在庫・取扱自治体は時期により変動します。本ページは楽天ふるさと納税の情報をもとに構成しており、'
        ...
        '20歳未満の飲酒・お酒の購入は法律で禁止されています。Terroir HUB はリキュールの販売を行っていません。</p>')


PUBLISHER = {"@type": "Organization", "name": "合同会社FOMUS",
             "url": f"https://{DOMAIN}/", "sameAs": ["https://liqueur.terroirhub.com/", "https://terroirhub.com/"]}
...
def jsonld_for(name, desc, url, crumb, items, faqs):
    ...
    g = [
        ...
        {"@type": "CollectionPage", "name": name, "description": desc, "url": url,
         ...
         "isPartOf": {"@type": "WebSite", "name": "Terroir HUB LIQUEUR", "url": f"https://{DOMAIN}/"}},
...
def build_hub(by_pref):
    ...
    budget_cards = "".join(
        f'<a class="bs-card" href="/liqueur/furusato/budget-{_k}.html">'
    ...
    for p, items in ordered:
        ...
        more = (f'<a class="pref-more" href="/liqueur/furusato/{p}.html">{esc(PREF_NAMES[p])}の返礼品をすべて見る ({len(items)}件) →</a>'
                if len(items) > 6 else
                f'<a class="pref-more" href="/liqueur/furusato/{p}.html">{esc(PREF_NAMES[p])}の特集ページ →</a>')
        sections += f'''
      <section class="pref-block" data-pref="{p}" id="pref-{p}">
        <h2 class="pref-title">{esc(PREF_NAMES[p])}のリキュールふるさと納税 <span class="pref-count">{len(items)}件</span>{more}</h2>
        <div class="fgrid">{cards}
        </div>
      </section>'''

    faqs = [("リキュールのふるさと納税でおすすめは？",
             f"Terroir HUB LIQUEURでは全国{n_pref}都道府県・{n_winery}メーカーのリキュールふるさと納税返礼品{total}件を産地別に掲載。山梨・長野・北海道・山形など主要産地から、純米・吟醸・大吟醸など多彩なリキュールを寄付で応援しながら受け取れます。")] + FAQ_COMMON
    desc = (f"リキュールのふるさと納税返礼品を産地別に{total}件掲載。全国{n_winery}メーカーの純米・吟醸・大吟醸を、"
            "寄付で蔵を応援しながら受け取れます。楽天ふるさと納税対応。")
    title = "リキュールのふるさと納税 返礼品 | 全国の蔵を応援 — Terroir HUB LIQUEUR"
    url = f"https://{DOMAIN}/liqueur/furusato/"
    crumb = [("Terroir HUB LIQUEUR", f"https://{DOMAIN}/"), ("ふるさと納税", url)]
    all_items = [it for _, v in ordered for it in v]
    jsonld = jsonld_for("リキュールのふるさと納税 返礼品", desc, url, crumb, all_items, faqs)

    head = HEAD.format(title=esc(title), desc=esc(desc), ogtitle=esc("リキュールのふるさと納税 返礼品｜全国の蔵を応援"),
                       canonical=url, jsonld=jsonld) + HEAD_TAIL
    faq_html = "".join(f'<div class="faq-item"><dt>{esc(q)}</dt><dd>{esc(a)}</dd></div>' for q, a in faqs)
    body = f'''
<header class="hero">
  <div class="hero-inner">
    <div class="hero-tag">FURUSATO TAX — ふるさと納税でリキュールを</div>
    <h1>寄付して、日本のメーカーを応援しよう。</h1>
    <p>ふるさと納税の返礼品としてリキュールを受け取りながら、産地と蔵を直接応援。Terroir HUBが全国のメーカーの返礼品を産地別に集めました。実質自己負担2,000円で、あなたの好きなリキュールを。</p>
    <div class="stats">
      <div class="stat"><div class="stat-n">{total}</div><div class="stat-l">返礼品</div></div>
      <div class="stat"><div class="stat-n">{n_winery}</div><div class="stat-l">メーカー</div></div>
...
<section class="steps"><div class="steps-grid">
  ...
  <div class="step"><div class="step-n">2</div><h3>好きなリキュールを選ぶ</h3><p>産地・メーカーから選んで「楽天ふるさと納税で見る」へ。寄付すると返礼品として届きます。</p></div>
...
<div class="toolbar"><div class="toolbar-inner">
  <input type="search" class="search-box" id="fsearch" placeholder="🔍 メーカー名・銘柄で絞り込む（例: 獺祭、純米大吟醸、新潟）">
...
{DEADLINE_BLOCK}
<section class="faqsec"><h2>ふるさと納税リキュールのよくある質問</h2><dl class="faq">{faq_html}</dl></section>
{NOTE}
{HUB_JS}
'''
...
def build_pref(pref, items, all_prefs):
    ...
    lead = PREF_LEAD.get(pref) or (
        f"{pn}のリキュールふるさと納税返礼品を{n}件掲載しています。"
        + f"{('、'.join(wineries[:3]))}など{n_winery}のメーカーの返礼品から、寄付で{pn}の地酒とメーカーを直接応援できます。")
    cards = "".join(card_html(it) for it in items)

    title = f"{pn}のリキュール ふるさと納税 返礼品{n}件 | Terroir HUB LIQUEUR"
    desc = (f"{pn}のリキュールふるさと納税返礼品を{n}件掲載（{n_winery}メーカー）。"
            f"{('、'.join(wineries[:4]))}など{pn}のリキュールを、寄付で応援しながら受け取れます。楽天ふるさと納税対応。")
    url = f"https://{DOMAIN}/liqueur/furusato/{pref}.html"
    crumb = [("Terroir HUB LIQUEUR", f"https://{DOMAIN}/"),
             ("ふるさと納税", f"https://{DOMAIN}/liqueur/furusato/"),
             (pn, url)]
    faqs = [
        (f"{pn}のリキュールでふるさと納税のおすすめは？",
         f"{pn}では{n_winery}のメーカーから{n}件のリキュール返礼品が選べます。{('、'.join(wineries[:5]))}などのリキュールが寄付で受け取れます。"),
        (f"{pn}のリキュールふるさと納税はどこで寄付できますか？",
         "各返礼品の「楽天ふるさと納税で見る」ボタンから楽天ふるさと納税の該当ページに移動して寄付できます。"),
    ] + FAQ_COMMON[:3]
    jsonld = jsonld_for(f"{pn}のリキュール ふるさと納税返礼品", desc, url, crumb, items, faqs)
    head = HEAD.format(title=esc(title), desc=esc(desc), ogtitle=esc(f"{pn}のリキュール ふるさと納税 返礼品{n}件"),
                       canonical=url, jsonld=jsonld) + HEAD_TAIL

    others = [p for p in all_prefs if p != pref]
    other_links = "".join(f'<a href="/liqueur/furusato/{p}.html">{esc(PREF_NAMES[p])}</a>' for p in others)
    faq_html = "".join(f'<div class="faq-item"><dt>{esc(q)}</dt><dd>{esc(a)}</dd></div>' for q, a in faqs)
    body = f'''
<div class="crumb"><a href="/">ホーム</a> › <a href="/liqueur/furusato/">ふるさと納税</a> › {esc(pn)}</div>
<header class="hero">
  <div class="hero-inner">
    <div class="hero-tag">FURUSATO TAX — {esc(pn)}</div>
    <h1>{esc(pn)}のリキュールを、ふるさと納税で。</h1>
    <p>{esc(lead)}</p>
    <div class="stats">
...
<main class="content">
  <section class="pref-block" data-pref="{pref}">
    <h2 class="pref-title">{esc(pn)}のリキュールふるさと納税 返礼品 <span class="pref-count">{n}件</span></h2>
    <div class="fgrid">{cards}
    </div>
    <p id="noresult" style="display:none;font-size:14px;color:#8a7a72;padding:20px 0;">該当する返礼品がありません。金額の条件を変えてお試しください。</p>
  </section>
  <div class="otherpref">
    <h2>{esc(pn)}のリキュールをもっと知る</h2>
    <div class="otherpref-list">
      <a href="/liqueur/{pref}/">{esc(pn)}のメーカー一覧</a>
      <a href="/liqueur/guide/">リキュールの基礎知識</a>
      <a href="/liqueur/search/">メーカー検索</a>
    </div>
  </div>
  <div class="otherpref">
    <h2>他の産地のリキュールふるさと納税</h2>
    <div class="otherpref-list">{other_links}</div>
  </div>
</main>
<section class="faqsec"><h2>{esc(pn)}のリキュールふるさと納税 よくある質問</h2><dl class="faq">{faq_html}</dl></section>
{NOTE}
'''
...
def build_budget(band_key, band_label, by_pref, all_prefs):
    ...
    「ふるさと納税 リキュール 1万円以下」のような金額指定の検索にも対応する。"""
    ...
    url = f"https://{DOMAIN}/liqueur/furusato/{slug}.html"
    title = f"{band_label}のリキュール ふるさと納税 返礼品{n}件 | Terroir HUB LIQUEUR"
    desc = (f"寄付金額{band_label}のリキュールふるさと納税返礼品を{n}件掲載（{n_pref}都道府県・{n_winery}メーカー）。"
            f"寄付額{lo:,}円〜{hi:,}円の返礼品を金額順に並べています。楽天ふるさと納税対応。")
    lead = (f"寄付金額が{band_label}のリキュール返礼品を{n}件集めました（{n_pref}都道府県・{n_winery}メーカー）。"
            f"控除上限額の範囲に収まるものを選べます。金額は掲載時点の楽天ふるさと納税の情報です。")
    crumb = [("Terroir HUB LIQUEUR", f"https://{DOMAIN}/"),
             ("ふるさと納税", f"https://{DOMAIN}/liqueur/furusato/"),
             (f"{band_label}", url)]
    faqs = [
        (f"{band_label}のリキュール返礼品は何件ありますか？",
         f"Terroir HUB LIQUEURでは{band_label}のリキュール返礼品を{n}件掲載しています（{n_pref}都道府県・{n_winery}メーカー）。"),
        ("寄付金額はどこの情報ですか？",
         "楽天ふるさと納税の掲載金額をもとにしています。金額・在庫・取扱自治体は変動するため、寄付前に各返礼品ページで最新の内容を確認してください。"),
    ] + FAQ_COMMON[:3]
    jsonld = jsonld_for(f"{band_label}のリキュール ふるさと納税返礼品", desc, url, crumb, items, faqs)
    head = HEAD.format(title=esc(title), desc=esc(desc),
                       ogtitle=esc(f"{band_label}のリキュール ふるさと納税 返礼品{n}件"),
                       canonical=url, jsonld=jsonld) + HEAD_TAIL

    others = "".join(
        f'<a href="/liqueur/furusato/budget-{k}.html">{esc(lb)}</a>'
        for k, lb, _lo, _hi in BUDGET_BANDS if k != band_key)
    pref_links = "".join(
        f'<a href="/liqueur/furusato/{p}.html">{esc(PREF_NAMES[p])}</a>' for p in all_prefs)
    cards = "".join(card_html(it) for it in items)
    faq_html = "".join(f'<div class="faq-item"><dt>{esc(q)}</dt><dd>{esc(a)}</dd></div>' for q, a in faqs)

    body = f'''
<div class="crumb"><a href="/">ホーム</a> › <a href="/liqueur/furusato/">ふるさと納税</a> › {esc(band_label)}</div>
<header class="hero">
  <div class="hero-inner">
    <div class="hero-tag">FURUSATO TAX — 寄付金額 {esc(band_label)}</div>
    <h1>{esc(band_label)}のリキュールを、ふるさと納税で。</h1>
    <p>{esc(lead)}</p>
    <div class="stats">
      <div class="stat"><div class="stat-n">{n}</div><div class="stat-l">返礼品</div></div>
      <div class="stat"><div class="stat-n">{n_winery}</div><div class="stat-l">メーカー</div></div>
...
<main class="content">
  <section class="pref-block">
    <h2 class="pref-title">{esc(band_label)}のリキュール返礼品 <span class="pref-count">{n}件</span></h2>
    <div class="fgrid">{cards}
    </div>
  </section>
  <div class="otherpref">
    <h2>ほかの寄付金額で見る</h2>
    <div class="otherpref-list">{others}<a href="/liqueur/furusato/">すべての返礼品</a></div>
  </div>
  <div class="otherpref">
    <h2>都道府県から探す</h2>
