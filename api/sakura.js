...
function getSearchIndex() {
  ...
  searchIndex = [];
  const dataDir = path.join(__dirname, '..', 'liqueur');
  const indexes = [
    { file: 'search_index.json', site: 'liqueur' },
...
★ サイト別デフォルトジャンル：
- このサイトのデフォルトジャンルは「リキュール」
- 「おすすめ教えて」「何かいいのある？」のようにジャンルが曖昧な質問は、リキュールとして回答する
- ユーザーが別ジャンルを明示した場合はそちらで回答する（「日本酒のおすすめ」等）
- 迷った場合はリキュールで回答。余計な提案をしない
