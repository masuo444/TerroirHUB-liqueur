// Terroir HUB LIQUEUR — レビューコンポーネント
// 使い方: メーカーページの末尾に <script src="/liqueur/reviews.js"></script> を追加
...
(function(){
  ...
  function getProducerId(){
    ...
    if(el && el.dataset.producerId) return el.dataset.producerId;
  ...
  style.textContent = `
    ...
    .rv-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid #F0D5E0;}
    ...
    .rv-field input,.rv-field textarea,.rv-field select{width:100%;padding:10px 12px;border:1.5px solid #F0D5E0;border-radius:8px;font-size:14px;font-family:'Noto Sans JP',sans-serif;outline:none;transition:border-color 0.2s;color:#333;background:#fff;}
    ...
    .rv-note-tag{font-size:12px;padding:4px 10px;border-radius:8px;background:#FDF0F4;color:#888;}
    ...
    .rv-loading-spinner{display:inline-block;width:24px;height:24px;border:3px solid #F0D5E0;border-top-color:${ACCENT};border-radius:50%;animation:rvSpin 0.8s linear infinite;}
  ...
  formOverlay.innerHTML = `
    <div class="rv-form" style="position:relative;">
      ...
      <div class="rv-field">
        <label>銘柄名</label>
        <input type="text" id="rvProductName" placeholder="例: The CHOYA、あらごし梅酒">
      </div>
      <div class="rv-field">
        <label>タイトル</label>
      ...
      <div class="rv-field">
        <label>香り</label>
        <input type="text" id="rvAroma" placeholder="例: 華やかな梅の香り、ゆずの爽やかな香り">
      </div>
      <div class="rv-field">
        <label>味わい</label>
        <input type="text" id="rvTaste" placeholder="例: 甘酸っぱい口当たり、フルーティー">
      </div>
      <div class="rv-field">
        <label>余韻</label>
        <input type="text" id="rvFinish" placeholder="例: すっきりとしたキレ、ほのかな甘み">
      </div>
      <div class="rv-field">
        <label>飲み方</label>
  ...
  const reviewTarget = document.getElementById('reviews-section');
  if (reviewTarget) {
    reviewTarget.appendChild(section);
  } else {
  ...
  const METHOD_LABELS = {
    on_the_rocks: 'ロック', sodawari: 'ソーダ割り', straight: 'ストレート',
    cocktail: 'カクテル', oyuwari: 'お湯割り', other: 'その他'
  };
