...
(function(){
  ...
  function showPwaPrompt(){
    ...
    modal.innerHTML = '<div style="background:#fff;border-radius:16px;max-width:400px;width:100%;padding:32px;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.15);">' +
      ...
      '<div style="background:#fafaf8;border-radius:10px;padding:14px;margin-bottom:16px;text-align:left;font-size:12px;color:#555;line-height:2;">' +
        ...
        '② 「ホーム画面に追加」を選択' +
      '</div>' +
      '<button onclick="this.closest(\'#pwa-prompt\').remove()" style="background:#4A8BAE;color:#fff;border:none;padding:10px 24px;border-radius:8px;font-size:14px;cursor:pointer;font-weight:500;">閉じる</button>' +
    '</div>';
    document.body.appendChild(modal);
  }
  ...
  function showSakuraAuthButtons(){
    ...
    div.innerHTML = '<a href="/api/line-login" style="display:flex;align-items:center;justify-content:center;gap:8px;background:#06C755;color:#fff;border:none;padding:12px 24px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;text-decoration:none;width:100%;max-width:280px;"><svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M12 2C6.48 2 2 5.82 2 10.5c0 4.01 3.56 7.37 8.36 8.17.33.07.78.22.89.5.1.26.07.66.03.92l-.14.87c-.04.26-.2 1.02.89.56s5.93-3.5 8.09-5.98C21.72 13.76 22 12.17 22 10.5 22 5.82 17.52 2 12 2z"/></svg>LINEでログイン</a>' +
      '<div style="display:flex;gap:8px;width:100%;max-width:280px;">' +
        '<button onclick="showAuth(\'signup\')" style="flex:1;background:#4A8BAE;color:#fff;border:none;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;">メールで登録</button>' +
        '<button onclick="showAuth(\'login\')" style="flex:1;background:none;border:1px solid #4A8BAE;color:#4A8BAE;padding:10px 16px;border-radius:8px;font-size:13px;cursor:pointer;">ログイン</button>' +
      '</div>';
    chat.appendChild(div);
    chat.scrollTop = chat.scrollHeight;
  }
  ...
  function showSakuraProButton(){
    ...
    var div = document.createElement('div');
    div.style.cssText = 'display:flex;flex-direction:column;gap:8px;align-items:center;padding:12px;';
    div.innerHTML = '<button onclick="if(window.thubAuth&&window.thubAuth.isLoggedIn){if(window.thubSubscribe)thubSubscribe(\'pro\')}else{sessionStorage.setItem(\'thub_pending_plan\',\'pro\');showAuth(\'signup\')}" style="background:#4A8BAE;color:#fff;border:none;padding:12px 28px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;">Proを始める（月額¥500）</button>' +
      '<a href="/liqueur/plans/" style="font-size:11px;color:#888;text-decoration:none;">プランの詳細を見る →</a>';
    chat.appendChild(div);
    chat.scrollTop = chat.scrollHeight;
  }
  ...
  function buildPackButton(credits, price){
    ...
    return '<button onclick="thubBuyCredits(' + credits + ')" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:14px 16px;border:1px solid #e8e4df;border-radius:10px;background:#fafaf8;cursor:pointer;transition:border-color 0.2s;" onmouseover="this.style.borderColor=\'#4A8BAE\'" onmouseout="this.style.borderColor=\'#e8e4df\'">' +
      ...
      '</div>' +
      '<div style="font-size:16px;font-weight:700;color:#4A8BAE;">&yen;' + price.toLocaleString() + '</div>' +
      '</button>';
  }
  ...
  function showLoginMessage(){
    ...
    modal.innerHTML = '<div style="background:#fff;border-radius:14px;max-width:440px;width:calc(100% - 32px);padding:28px;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.12);">' +
      ...
      '<button onclick="this.closest(\'#login-modal\').remove();showAuth(\'signup\');" style="background:#4A8BAE;color:#fff;border:none;padding:10px 24px;border-radius:8px;font-size:13px;cursor:pointer;font-weight:500;">無料会員登録（30秒）</button>' +
      '<div style="margin-top:10px;"><button onclick="this.closest(\'#login-modal\').remove();showAuth(\'login\');" style="background:none;border:none;color:#4A8BAE;font-size:12px;cursor:pointer;">ログインはこちら</button></div>' +
  ...
  function showFreeMessage(){
    ...
    modal.innerHTML = '<div style="background:#fff;border-radius:14px;max-width:440px;width:calc(100% - 32px);padding:28px;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.12);">' +
      ...
      '<a href="#plans" onclick="this.closest(\'#free-msg-modal\').remove()" style="display:inline-block;background:#4A8BAE;color:#fff;padding:10px 24px;border-radius:8px;text-decoration:none;font-size:13px;cursor:pointer;font-weight:500;">Proプラン（月額¥500）を見る</a>' +
  ...
  function injectProNudge(){
    ...
    nudge.style.cssText = 'display:none;padding:6px 14px;background:linear-gradient(90deg,rgba(74,139,174,0.06),rgba(212,114,138,0.06));border-top:1px solid rgba(74,139,174,0.1);';
    nudge.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">' +
      '<div style="font-size:11px;color:#888;line-height:1.5;">' +
      '<span style="color:#4A8BAE;font-weight:600;">Pro</span>にすると、AIサクラがあなた専用のソムリエに' +
      '</div>' +
      '<button onclick="var m=this.closest(\'.overlay\');if(m)m.classList.remove(\'open\');if(window.thubAuth&&window.thubAuth.isLoggedIn){if(typeof thubSubscribe===\'function\')thubSubscribe(\'pro\')}else{if(typeof showAuth===\'function\')showAuth(\'signup\')}" style="flex-shrink:0;background:#4A8BAE;color:#fff;border:none;padding:5px 12px;border-radius:6px;font-size:10px;font-weight:600;cursor:pointer;white-space:nowrap;">月¥500</button>' +
      '</div>';
  ...
  function showCreditModal(plan, credits){
    ...
    if (plan === 'pro') {
      ...
      choicesHtml = '<div style="display:flex;flex-direction:column;gap:12px;margin-top:20px;">' +
        '<button onclick="this.closest(\'#credit-modal\').remove();if(typeof thubSubscribe===\'function\')thubSubscribe(\'premium\')" style="width:100%;padding:16px;background:linear-gradient(135deg,#4A8BAE,#8B3520);color:#fff;border:none;border-radius:12px;cursor:pointer;text-align:left;">' +
        ...
        '</button>' +
        '<button onclick="this.closest(\'#credit-modal\').remove();thubShowCreditShop();" style="width:100%;padding:16px;background:#fff;border:1.5px solid #4A8BAE;border-radius:12px;cursor:pointer;text-align:left;">' +
          '<div style="font-size:15px;font-weight:700;color:#4A8BAE;">クレジットを追加購入</div>' +
          '<div style="font-size:12px;color:#888;margin-top:4px;">10回¥300〜 使い切るまで有効</div>' +
        '</button>' +
      '</div>';
    } else if (plan === 'premium') {
      choicesHtml = '<div style="margin-top:20px;">' +
        '<button onclick="this.closest(\'#credit-modal\').remove();thubShowCreditShop();" style="width:100%;padding:16px;background:#4A8BAE;color:#fff;border:none;border-radius:12px;cursor:pointer;">' +
  ...
  function showTextbookGate(type, chapter){
    ...
    const content = type === 'login'
      ? `<div style="font-family:'Shippori Mincho',serif;font-size:18px;font-weight:600;color:#333;margin-bottom:8px;">ログインが必要です</div>
         ...
         <button onclick="this.closest('#textbook-gate').remove();showAuth('login');" style="background:#4A8BAE;color:#fff;border:none;padding:10px 24px;border-radius:8px;font-size:13px;cursor:pointer;font-weight:500;">ログイン / 無料登録</button>`
      : `<div style="font-family:'Shippori Mincho',serif;font-size:18px;font-weight:600;color:#333;margin-bottom:8px;">Pro プランで読めます</div>
         ...
         <a href="#plans" onclick="this.closest('#textbook-gate').remove()" style="display:inline-block;background:#4A8BAE;color:#fff;padding:10px 24px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">Proプラン（月額¥500）を見る</a>`;
  ...
  window.thubLogSake = function(breweryId, breweryName, brandName){
    ...
    modal.innerHTML = `<div style="background:#fff;border-radius:14px;max-width:440px;width:calc(100% - 32px);padding:28px;box-shadow:0 16px 48px rgba(0,0,0,0.12);">
      ...
      <div style="display:flex;gap:8px;">
        <button id="log-submit-btn" data-brewery-id="${escHtml(breweryId)}" data-brewery-name="${escHtml(breweryName)}" style="flex:1;background:#4A8BAE;color:#fff;border:none;padding:12px;border-radius:8px;font-size:14px;cursor:pointer;font-weight:500;">記録する</button>
  ...
  window.setLogStar = function(n){
    ...
    document.querySelectorAll('#log-stars span').forEach(function(s){
      ...
      s.style.color = parseInt(s.dataset.star) <= n ? '#4A8BAE' : '#ddd';
    });
  };
    ...
    if(logs.length === 0){
      ...
      content += logs.slice(0, 30).map(function(l){
        ...
        return '<div style="padding:12px 0;border-bottom:1px solid #f0f0f0;">' +
          ...
          '</div>' +
          (l.brand ? '<div style="font-size:13px;color:#4A8BAE;margin-top:2px;">' + escHtml(l.brand) + '</div>' : '') +
          '<div style="color:#4A8BAE;font-size:14px;margin-top:2px;">' + stars + '</div>' +
      ...
      content = list.map(function(w){
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #f0f0f0;">' +
          '<div style="flex:1;">' +
            ...
            (w.brand ? '<div style="font-size:12px;color:#4A8BAE;">' + escHtml(w.brand) + '</div>' : '') +
          '</div>' +
  ...
  function showBadgeNotification(badge){
    ...
    notif.style.cssText = 'position:fixed;top:70px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#4A8BAE,#6BAFD0);color:#fff;padding:16px 24px;border-radius:12px;z-index:900;box-shadow:0 8px 32px rgba(74,139,174,0.3);display:flex;align-items:center;gap:12px;animation:fadeInUp 0.5s ease;';
  ...
  function showXPToast(msg){
    ...
    toast.style.cssText = 'position:fixed;bottom:120px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#4A8BAE,#6BAFD0);color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;font-weight:600;z-index:850;animation:fadeInUp 0.3s ease;box-shadow:0 4px 16px rgba(74,139,174,0.3);';
    toast.textContent = msg;
    document.body.appendChild(toast);
  ...
  function showLevelUp(level){
    ...
    overlay.innerHTML = '<div style="background:#fff;border-radius:20px;padding:40px 32px;text-align:center;max-width:440px;box-shadow:0 24px 64px rgba(0,0,0,0.2);" onclick="event.stopPropagation()">' +
      ...
      '<div style="font-size:11px;letter-spacing:0.2em;color:#4A8BAE;font-weight:600;margin-bottom:8px;">LEVEL UP!</div>' +
      '<div style="font-family:\'Shippori Mincho\',serif;font-size:28px;font-weight:700;color:#333;margin-bottom:4px;">Lv.' + level.lv + '</div>' +
      '<div style="font-size:18px;color:#4A8BAE;font-weight:600;margin-bottom:16px;">' + level.name + '</div>' +
      '<div style="font-size:13px;color:#888;">おめでとうございます！</div>' +
      '<button onclick="this.closest(\'div\').parentElement.remove()" style="margin-top:20px;background:#4A8BAE;color:#fff;border:none;padding:10px 32px;border-radius:10px;font-size:14px;cursor:pointer;">続ける</button>' +
    '</div>';
...
(function(){
  ...
  Promise.all([
    ...
    html += '<div style="font-size:9px;letter-spacing:0.35em;color:#4A8BAE;text-transform:uppercase;margin-bottom:8px;">Community</div>';
    ...
    if(logs.length > 0){
      ...
      if(rated.length > 0){
        ...
        html += '<div style="font-size:32px;font-weight:700;color:#4A8BAE;font-family:Inter,sans-serif;">' + avg + '</div>';
        html += '<div><div style="color:#6BAFD0;font-size:16px;">';
      ...
      logs.forEach(function(l){
        var stars = '';
        if(l.rating > 0) for(var i=1;i<=5;i++) stars += '<span style="color:'+(i<=l.rating?'#6BAFD0':'#e5e5e5')+';">★</span>';
    ...
    html += '<a href="https://terroirhub.com/quest/" style="display:inline-flex;align-items:center;gap:6px;font-size:13px;color:#4A8BAE;font-weight:500;text-decoration:none;border:1px solid rgba(74,139,174,0.25);padding:8px 20px;border-radius:8px;">テロワールクエストで記録する →</a>';
    html += '</div></section>';
...
(function(){
  ...
  style.textContent = `
    ...
    .atlas-resize:hover,.atlas-resize.active{background:rgba(74,139,174,0.15);}
    body.atlas-open{margin-right:380px;transition:margin-right 0.3s;}
    body.atlas-closed{margin-right:0;}
    .atlas-toggle{position:fixed;right:16px;top:12px;z-index:101;width:40px;height:40px;background:linear-gradient(135deg,#6BAFD0,#4A8BAE);border:none;border-radius:50%;color:#fff;font-size:18px;cursor:pointer;box-shadow:0 4px 16px rgba(74,139,174,0.3);display:flex;align-items:center;justify-content:center;transition:transform 0.2s;}
    ...
    .atlas-close{background:#f5f2ec;border:none;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:16px;color:#4A8BAE;display:flex;align-items:center;justify-content:center;transition:background 0.15s;}
    ...
    .atlas-msg.bot .atlas-bubble{background:#fff;border:1px solid rgba(107,175,208,0.12);}
    ...
    .atlas-inp textarea:focus{border-color:#6BAFD0;}
    .atlas-inp button{background:#4A8BAE;border:none;color:#fff;width:36px;height:36px;border-radius:8px;cursor:pointer;font-size:15px;font-weight:700;flex-shrink:0;}
  `;
  document.head.appendChild(style);
  ...
  panel.innerHTML = `
    ...
    <div class="atlas-hdr">
      <div class="atlas-hdr-l">
        <img class="atlas-av" src="/liqueur/sakura.jpg" alt="サクラ">
        <div>
    ...
    <div class="atlas-inp">
      <textarea id="atlas-input" rows="1" placeholder="リキュールについて何でも…" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();atlasSend();}"></textarea>
  ...
  function addAtlasMsg(role, text){
    ...
    var av = role === 'bot' ? '<div class="atlas-mav"><img src="/liqueur/sakura.jpg" alt=""></div>' : '';
  ...
  if(history.length > 0){
    history.forEach(function(m){
      ...
      var av = m.role === 'bot' ? '<div class="atlas-mav"><img src="/liqueur/sakura.jpg" alt=""></div>' : '';
      d.innerHTML = av + '<div class="atlas-bubble">' + escHtmlAtlas(m.text).replace(/\n/g,'<br>') + '</div>';
      chat.appendChild(d);
    });
    chat.scrollTop = chat.scrollHeight;
  } else {
    addAtlasMsg('bot', 'こんにちは、サクラです。🌸\n\nリキュールのことなら何でも聞いてくださいね。メーカーの情報、銘柄のおすすめ、料理とのペアリングまで。');
  }
  renderAtlasSugs();
  ...
  window.atlasSend = function(){
    ...
    tp.innerHTML = '<div class="atlas-mav"><img src="/liqueur/sakura.jpg" alt=""></div><div class="atlas-bubble"><span style="color:#ccc;">考え中...</span></div>';
